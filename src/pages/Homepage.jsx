...
const HeroSection = ({ inventorylink }) => (
  <div className="bg-white py-8 px-4 sm:px-6 lg:px-8 rounded-lg shadow-lg mb-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between">
        <div className="md:w-1/2 mb-8 md:mb-0">
          ...
          <div className="space-y-4 sm:space-y-0 sm:space-x-4 flex flex-col sm:flex-row">
            <button className="bg-orange-500 text-white px-6 py-2 rounded-md hover:bg-orange-600 transition duration-300 w-full sm:w-auto">
              ADD TO CART
            </button>
            <button onClick={inventorylink} className="border border-orange-500 text-orange-500 px-6 py-2 rounded-md hover:bg-orange-50 transition duration-300 w-full sm:w-auto">
              VIEW DETAIL
            </button>
          </div>
        </div>
...
const CategoryMenu = ({ categories, selectedCategory, setSelectedCategory }) => (
  <div className="flex flex-wrap justify-center gap-4 mb-8 text-base sm:text-lg font-bold text-gray-700">
...
const ItemsGrid = ({ items, searchQuery, selectedCategory, addToCart }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 bg-orange-100 p-4 sm:p-6 rounded-lg mb-8">
    {items
        ...
        <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden transform hover:scale-105 transition duration-300">
          ...
          <div className="p-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-2">
              <span className="font-bold text-orange-600 mb-2 sm:mb-0">Rs {item.price.toFixed(2)}</span>
              {item.stock > 0 ? (
                <button
                  onClick={() => addToCart(item.id, item.stock)}
                  className="bg-orange-600 text-white px-4 py-2 rounded hover:bg-blue-600 w-full sm:w-auto"
                >
                  Add to Cart
                </button>
              ) : (
...
const ServicesSection = ({ services }) => (
  <div className="bg-orange-50 py-8 sm:py-12 rounded-lg mb-8">
    ...
    <p className="text-center text-gray-600 mb-10 max-w-xl mx-auto px-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-4 sm:px-8 lg:px-16">
      {services.map((service, index) => (
        <div key={index} className="bg-white p-6 sm:p-8 rounded-lg shadow-lg flex flex-col items-center text-center">
...
const ReservationSection = () => (
  <div className="bg-white p-6 sm:p-8 rounded-lg shadow-lg mb-8">
    ...
    <div className="flex justify-center">
      <button className="bg-orange-600 text-white px-6 py-2 rounded-lg hover:bg-orange-700 w-full sm:w-auto">
        Make Reservation
      </button>
    </div>
  </div>
